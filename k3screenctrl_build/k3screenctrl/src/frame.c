...
int frame_send(const unsigned char *data, int len) {
     ...
     * len*2 covers the worst case where every byte is escaped; +8 covers
     * the header, trailer, 2-byte CRC and a safety margin. The original
     * len*2+4 was too small and caused a heap overflow. */
