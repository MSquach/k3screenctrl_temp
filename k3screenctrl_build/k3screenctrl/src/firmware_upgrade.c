...
/*
 * Hardware pins for the screen MCU. These mirror the definitions in main.c.
 * Kept here so firmware_upgrade.c is self-contained.
 */
 ...
 * reported major version, the upgrade is refused: different major
 * versions mean different architectures (e.g. 1.x = MIPS, 2.x = ARM)
...
/*
 * frame_send(data, len) sends `data` verbatim as the payload and computes
 * the CRC over it. The first byte of `data` therefore plays the role of the
 * frame type. To send a bootloader command we prepend the type byte to the
 * payload, which produces the same wire format as frame_send(type, data, len).
 */
